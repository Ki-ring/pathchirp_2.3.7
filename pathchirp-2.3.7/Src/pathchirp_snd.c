...
/*
 * "pathchirp_snd" sends chirp packet trains to "pathchirp_rcv" based
 * on specified parameters.
 *
 *  pathchirp_snd.c is based on udpsend.c of the NetDyn tool.
 * */
...
int usage()
{
   (void) fprintf (stderr, "usage: pathchirp_snd [-h] <more options>\n");
     ...
     char	*argv[];
{
  ...
  while (argc > 0) {
	...
	*ptr = 0;
    ...
    default:
      (void) fprintf (stderr,
		      "pathchirp_snd: Unknown option '%c'\n", ptr[-1]);
      (void) exit (1);
...
void connect_so()
{
  ...
  if (connect (soudp, (struct sockaddr *) &dst, sizeof (dst)) < 0) {
      perror ("pathchirp_snd: could not connect to the receiver");
      (void) exit (1);
   }
...
void unconnect_so()
{
   ...
   connect (soudp, (struct sockaddr *) &dst, sizeof (dst));
      ...
      perror ("pathchirp_snd: could not unconnect\n");
      (void) exit (1);
      }*/
...
inline void send_pkt()
{
  ...
  if (cc < 0) {
    (void) fprintf (stderr,
		    "Packet number %d\n", np);
    if(debug) fprintf(stderr,"pathchirp_snd: write/sendto error, resetting\n");
    unconnect_so();
  }
...
int compute_parameters()
{
  ...
  chirp_duration=0;
  interarrival=((double)pktsize)*8.0*(double)jumbo/low_rate;
  largest_inter_arrival=interarrival/(double)jumbo;

  for(count=1;count<=num_interarrival;count++)
    {
...
void recv_pkt()
{
  ...
  if (cc < 0) {
    fprintf (stderr,"pathchirp_snd: read,connected=%d",connected);
    unconnect_so();
    return;
  } 
  ...
  else{  if(debug) fprintf(stderr,"crc BAD\n");}
...
void chirps_snd()
{
  ...
  sleeptime = largest_inter_arrival*spread_factor;
