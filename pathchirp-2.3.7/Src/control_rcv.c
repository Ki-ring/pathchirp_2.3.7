...
void send_pkt(int state)
{
  ...
  switch(state)
    ...
    default:
      perror("pathchirp_rcv: error in case of send_pkt\n");
      exit(0);
      break;
    }
...
void recv_chall_pkt()
{
  ...
  if (check_crc_snd2rcv(udprecord))
	...
	state=CHALL_REPLY;
      ...
      else{if(debug) fprintf(stderr,"req num wrong\n");}
    }
...
void initiate_connection()
{
 ...
 double tmp_var=0;/* variable used to create message to sender  */
  ...
  if (setsockopt(soudp, SOL_SOCKET, SO_REUSEADDR,(char *) &flag_on_recv,
    ...
    perror ("pathchirp_rcv: setsockopt failed");
      (void) exit (1);
  }
  ...
  if (setsockopt(soudp, SOL_SOCKET, SO_RCVBUF, (char *)&rcv_size,
		 sizeof (rcv_size)) < 0) {
    perror ("pathchirp_rcv: receive buffer option");
    exit (1);
  }
  ...
  if (setsockopt(soudp, SOL_SOCKET, SO_TIMESTAMP,&flag_on_recv, sizeof(flag_on_recv))<0){
      perror ("pathchirp_rcv: setsockopt SO_TIMESTAMP failed");
      (void) exit (1);
      }
#endif
  ...
  /* connect so that we only receive packets from sender */
    ...
    perror ("pathchirp_rcv: could not connect to the sender");
   (void) exit (1);
  }
    else{
